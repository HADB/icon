...
import {
  parseSVGContent,
  convertParsedSVG,
} from "@iconify/utils/lib/svg/parse";
...
const KEYWORDS_EDGE_TARGETS: string[] = ["edge", "cloudflare", "worker"];
...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    const templateServer = addTemplate({
      ...
      async getContents() {
        ...
        for (const collection of collections) {
          const prefix =
            typeof collection === "string" ? collection : collection.prefix;
        ...
        function getRemoteEndpoint(name: string) {
          if (typeof remote === "function") return remote(name);
        ...
        const collectionsValues = collections.map((collection) => {
          if (typeof collection === "string") {
            if (remote) {
              return `  '${collection}': createRemoteCollection(${JSON.stringify(
                getRemoteEndpoint(collection)
              )}),`;
            }
            ...
            if ("fetchEndpoint" in collection)
              return `  '${prefix}': createRemoteCollection(${JSON.stringify(
                collection.fetchEndpoint
              )}),`;
    ...
    const iconifyCollectionMap = new Map<
      string,
    ...
    addTemplate({
      ...
      async getContents() {
        ...
        if (!icons.length) return "export function init() {}";
        ...
        lines.push(
          ...
          ...(await Promise.all(
            icons.map(async (icon) => {
              ...
              if (collection) data = getIconData(collection, name);

              if (!data) {
                logger.error(
                  `Nuxt Icon could not fetch the icon data for \`${icon}\``
                );
    ...
    if (hasNuxtModule("@unocss/nuxt")) unocssIntegration(nuxt, options);

    await nuxt.callHook("icon:serverKnownCssClasses", serverKnownCssClasses);
  },
});

async function discoverLocalCollections(): Promise<
  ServerBundleOptions["collections"]
> {
  const collections = isFullCollectionExists
    ? collectionNames
    : collectionNames.filter((collection) =>
        isPackageExists("@iconify-json/" + collection)
      );
  if (isFullCollectionExists)
    logger.success(
      `Nuxt Icon discovered local-installed ${collections.length} collections (@iconify/json)`
    );
  else if (collections.length)
    logger.success(
      `Nuxt Icon discovered local-installed ${collections.length} collections:`,
      collections.join(", ")
    );
  return collections;
}
...
async function resolveServerBundle(
  ...
  if (resolved.disabled && customCollections.length)
    logger.warn(
      "Nuxt Icon server bundle is disabled, the custom collections will not be bundled."
    );
  ...
  return {
    ...
    collections: await Promise.all(
      [...(resolved.collections || []), ...customCollections].map((c) =>
        resolveCollection(nuxt, c)
      )
    ),
  };
}
  ...
  if ("dir" in collection) {
    ...
    const parsedIcons = await Promise.all(
      files.map(async (file) => {
        ...
        if (!data) {
          logger.error(
            `Nuxt Icon could not parse the SVG content for icon \`${name}\``
          );
          return [name, {}];
        }
        if (data.top === 0) delete data.top;
        if (data.left === 0) delete data.left;
        return [name, data];
      })
    );

    const successfulIcons = parsedIcons.filter(
    ...
    logger.success(
      `Nuxt Icon loaded local collection \`${collection.prefix}\` with ${successfulIcons.length} icons`
    );
    return {
      ...collection,
